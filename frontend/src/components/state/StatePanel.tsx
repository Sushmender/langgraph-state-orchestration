  ...
  return (
    <div className="flex flex-col h-full glass rounded-xl overflow-hidden">
      ...
      <div className="flex items-center border-b border-[rgba(122,17,40,0.12)] bg-[#1c1414]/60">
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive ? 'text-[#9c1a37]' : 'text-[#8f7f7c] hover:text-[#e8e0da]'
                )}
              >
                ...
                {isActive && (
                  <motion.div
                    ...
                    className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#7a1128]"
                  />
                )}
              </button>
            );
        ...
        {/* Edit button — lives in the tab bar, never covers content */}
        {canEditCurrentTab && (
          <button
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {activeTab === 'raw' ? (
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {stateValues.queries.map((q, i) => (
                        <span key={i} className="text-xs px-2.5 py-1 rounded-lg bg-[#7a1128]/10 border border-[rgba(122,17,40,0.2)] text-[#9c1a37]">
                          {q}
                        </span>
                      ))}
                    </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2 max-h-72 overflow-y-auto">
                      {stateValues.content.map((c, i) => (
                        <div key={i} className="p-3 rounded-lg bg-[#231a1a]/70 border border-[rgba(122,17,40,0.1)]">
                          <p className="text-xs text-[#8f7f7c] leading-relaxed">{c.slice(0, 300)}{c.length > 300 ? '…' : ''}</p>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              /* Plan / Draft / Critique — markdown only, no floating button */
              (() => {
